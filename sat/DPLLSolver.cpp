...
//dfs写法
//这里的result层考虑过也每次都拷贝一个副本用于严格的保存答案，但是仔细想想发现我们只需得到一个解就结束整个DPLL过程，那么最终的result一定是保存了答案序列的，但是别的变元取的值可能改变过（显然），但是这并不会影响最终的答案，所以为了性能，并没有每次都拷贝result，当然如果为了得到完美的答案（即在最终的答案result中体现到底是哪些变元决定了这个解），拷贝result是值得的
status DPLL(HeadNode *LIST,consequence *result) {
    ...
    //分裂策略
    int Var = LIST->right->data;//选取变元,直接第一行子句的第一个节点,这里可以考虑优化为按频率选取变元，当然可能效果不好
